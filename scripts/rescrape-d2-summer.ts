/**
 * Re-scrape D2 2025 Summer — fill in missing games
 */
import * as https from 'https'
import { prisma } from '../lib/db'

function fetchUrl(url: string, opts?: {method?:string;body?:string;headers?:Record<string,string>}): Promise<string> {
  return new Promise(async (resolve, reject) => {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (attempt === maxRetries - 1) reject(e)
      }
    }
  })
}
...
interface PS {
  ...
  points:number;assists:number;rebounds:number;steals:number;blocks:number
}

function parseBoxScore(html: string, gameId: string) {
  ...
  function parseTable(content: string): PS[] {
    ...
    while ((rowM = rowRe.exec(content)) !== null) {
      ...
      stats.push({
        ...
        rebounds: parseInt0(tds[16]), steals: tds.length > 17 ? parseInt0(tds[17]) : 0,
  ...
  return { awayTeam, homeTeam, awayScore, homeScore, date, awayStats: tables[0] ? parseTable(tables[0]) : [], homeStats: tables[1] ? parseTable(tables[1]) : [] }
}

async function main() {
  const LEAGUE_ID = '65672'
  const SEASON_ID = '105128'
  const SEASON_LABEL = 'D2 2025 Summer'
  const LEAGUE = 'Rec'

  console.log('🔁 Re-scraping D2 2025 Summer...')

  // Get all game IDs for this season via POST
  const baseHtml = await fetchUrl(`https://www.mystatsonline.com/basket/visitor/league/schedule_scores/schedule.aspx?IDLeague=${LEAGUE_ID}`)
  ...
  const schedHtml = await fetchUrl(`https://www.mystatsonline.com/basket/visitor/league/schedule_scores/schedule.aspx?IDLeague=${LEAGUE_ID}`, {
    method: 'POST', body: params.toString(),
    headers: { Referer: `https://www.mystatsonline.com`, Origin: 'https://www.mystatsonline.com' }
  })

  const allGameIds = parseGameIds(schedHtml)
  console.log(`Found ${allGameIds.length} total game IDs`)

  // Find which are already imported (check by looking for games with same season)
  const existingGames = await prisma.game.findMany({ where: { season: SEASON_LABEL }, select: { id: true, homeTeamId: true, awayTeamId: true } })
  console.log(`Already have ${existingGames.length} games in DB`)
  ...
  async function getTeam(name: string): Promise<string> {
    ...
    const slug = slugify(`${name}-d2-2025-summer`)
    let t = await prisma.team.findFirst({ where: { name, league: LEAGUE } })
    if (!t) {
      const existing = await prisma.team.findUnique({ where: { slug } })
      t = await prisma.team.create({ data: { name, slug: existing ? `${slug}-${Date.now()}` : slug, league: LEAGUE, color: '#4A9FE3' } })
    }
  ...
  for (let i = 0; i < allGameIds.length; i++) {
    ...
    try {
      ...
      if (!box.awayTeam || !box.homeTeam) { console.log(`  ⚠️  Game ${gameId}: no teams`); continue }

      const awayTeamId = await getTeam(box.awayTeam)
      const homeTeamId = await getTeam(box.homeTeam)

      let game = await prisma.game.findFirst({ where: { homeTeamId, awayTeamId, season: SEASON_LABEL } })
      if (!game) {
        game = await prisma.game.create({
          data: { date: box.date, location: 'Irving Masjid', season: SEASON_LABEL, league: LEAGUE, played: true, homeTeamId, awayTeamId, homeScore: box.homeScore, awayScore: box.awayScore, week: i+1 }
        })
        imported++
      } else {
        await prisma.game.update({ where: { id: game.id }, data: { homeScore: box.homeScore, awayScore: box.awayScore, played: true } })
      }
      ...
      for (const { teamId, stats } of [{ teamId: awayTeamId, stats: box.awayStats }, { teamId: homeTeamId, stats: box.homeStats }]) {
        for (const ps of stats) {
          ...
          const statData = { twoPtMade: ps.twoPtMade, twoPtAtt: ps.twoPtAtt, threeMade: ps.threeMade, threeAtt: ps.threeAtt, ftMade: ps.ftMade, ftAtt: ps.ftAtt, points: ps.points, assists: ps.assists, rebounds: ps.rebounds, steals: ps.steals, blocks: ps.blocks, turnovers: 0 }
          if (!existing) await prisma.playerGameStat.create({ data: { playerId, gameId: game.id, teamId, ...statData } })
  ...
  console.log(`\n✅ Done. ${finalCount} total games for ${SEASON_LABEL} (added ${imported} new)`)
