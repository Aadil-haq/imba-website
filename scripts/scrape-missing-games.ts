/**
 * Scrape missing games for D2 2025-26 Winter and D1 2025-26 Winter
 * Run: npx ts-node --compiler-options '{"module":"CommonJS"}' scripts/scrape-missing-games.ts
 */
...
async function scrapeAndImport(seasonId: string, seasonLabel: string, league: string) {
  console.log(`\n📋 Processing: ${seasonLabel} (ID: ${seasonId})`)

  // Step 1: Get VIEWSTATE from the schedule page
  ...
  // Step 2: POST to load the specific season
  ...
  const schedHtml = await fetchUrl(
    `https://www.mystatsonline.com/basket/visitor/league/schedule_scores/schedule.aspx?IDLeague=${LEAGUE_ID}`,
    { method: 'POST', body: params.toString(), headers: { Referer: 'https://www.mystatsonline.com', Origin: 'https://www.mystatsonline.com' } }
  )

  const allGameIds = parseGameIds(schedHtml)
  console.log(`   Found ${allGameIds.length} total game IDs on schedule page`)

  // Step 3: Find existing games in DB
  const existingGames = await prisma.game.findMany({
    where: { season: seasonLabel },
  ...
  async function getTeam(name: string): Promise<string> {
    ...
    const slug = slugify(`${name}-${seasonLabel}`)
  ...
  for (let i = 0; i < allGameIds.length; i++) {
    ...
    try {
      ...
      if (!box.awayTeam || !box.homeTeam) {
        console.log(`  ⚠️  Game ${gameId}: could not parse teams, skipping`)
        continue
      }
      ...
      let game = await prisma.game.findFirst({ where: { homeTeamId, awayTeamId, season: seasonLabel } })
      ...
      if (!game) {
        ...
        imported++
        console.log(`  ✅ NEW [${i + 1}/${allGameIds.length}] ${box.awayTeam} ${box.awayScore} @ ${box.homeTeam} ${box.homeScore} (${box.date.toDateString()})`)
      } else {
        ...
        console.log(`  ↻  exists [${i + 1}/${allGameIds.length}] ${box.awayTeam} ${box.awayScore} @ ${box.homeTeam} ${box.homeScore}`)
      }
  ...
  const finalCount = await prisma.game.count({ where: { season: seasonLabel } })
  console.log(`\n   ✅ Done. ${finalCount} total games for ${seasonLabel} (added ${imported} new)`)
}

async function main() {
  await scrapeAndImport('107276', 'D2 2025-26 Winter', 'Rec')
  await scrapeAndImport('107277', 'D1 2025-26 Winter', 'Comp')
